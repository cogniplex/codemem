...
fn build_server() -> anyhow::Result<(codemem_mcp::McpServer, codemem_storage::Storage)> {
...
pub(crate) fn cmd_serve(api: bool, http: bool, port: u16) -> anyhow::Result<()> {
    ...
    match (api, http) {
        ...
        (true, false) => {
            ...
            let api_server = codemem_api::ApiServer::new(Arc::clone(&server), api_storage);
            ...
            Ok(()) => {
                ...
                if let Some(sid) = payload.session_id.as_deref() {
                    if !sid.is_empty() {
                        ...
                        let auto_insights = codemem_hooks::check_triggers(
                            &storage,
                            sid,
                            &tool_name,
                            file_path_owned.as_deref(),
                        ...
                        for insight in &auto_insights {
                            let insight_hash = codemem_hooks::content_hash(&insight.content);
    ...
    loop {
        ...
        if let Some(event) = event {
            let (path, event_type) = match &event {
                codemem_watch::WatchEvent::FileChanged(p) => (p.clone(), "modified"),
                codemem_watch::WatchEvent::FileCreated(p) => (p.clone(), "created"),
                codemem_watch::WatchEvent::FileDeleted(p) => (p.clone(), "deleted"),
            };

            let language = codemem_watch::detect_language(&path)
                .unwrap_or("unknown")
                .to_string();
...
fn flush_batch(
    ...
    vector: &mut codemem_vector::HnswIndex,
    quiet: bool,
) -> usize {
    if batch.is_empty() {
        return 0;
