...
pub async fn indexing_events(State(state): State<Arc<AppState>>) -> impl IntoResponse {
...
pub async fn watch_events(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let rx = state.watch_events.subscribe();
    let stream = BroadcastStream::new(rx).filter_map(|result| match result {
        Ok(event) => {
            let (path, event_type) = match &event {
                codemem_engine::watch::WatchEvent::FileChanged(p) => {
                    (p.to_string_lossy().to_string(), "changed")
                }
                codemem_engine::watch::WatchEvent::FileCreated(p) => {
                    (p.to_string_lossy().to_string(), "created")
                }
                codemem_engine::watch::WatchEvent::FileDeleted(p) => {
