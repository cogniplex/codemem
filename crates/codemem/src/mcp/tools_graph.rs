...
impl McpServer {
    ...
    pub(crate) fn tool_index_codebase(&self, args: &Value) -> ToolResult {
        ...
        let all_chunks = resolved.chunks;
        let seen_files = resolved.file_paths;
        ...
        for file_path in &seen_files {
            ...
            let _ = self.engine.storage.insert_graph_node(&node);
        ...
        for file_path in &seen_files {
            ...
            if let Some(last_dir) = ancestors.last() {
                ...
                let edge = Edge {
                    ...
                    dst: file_node_id,
                    relationship: codemem_core::RelationshipType::Contains,
                    weight: edge_weight_for(&RelationshipType::Contains, &self.engine.config.graph),
                    valid_from: None,
                    valid_to: None,
                    properties: HashMap::new(),
                    created_at: now,
        ...
        for sym in &all_symbols {
            ...
            let _ = self.engine.storage.insert_graph_edge(&contains_edge);
        ...
        for edge in &edges {
            let e = Edge {
                ...
                weight: edge_weight_for(&edge.relationship, &self.engine.config.graph),
                valid_from: None,
                valid_to: None,
                properties: HashMap::new(),
                created_at: now,
            };
            let _ = self.engine.storage.insert_graph_edge(&e);
        ...
        let mut chunk_count = 0usize;
        {
            for file_path in &seen_files {
                ...
                let _ = self.engine.storage.delete_graph_nodes_by_prefix(&prefix);
            }
            ...
            for chunk in &all_chunks {
                ...
                let _ = self.engine.storage.insert_graph_edge(&file_chunk_edge);
        ...
        if let Some(emb) = match self.lock_embeddings() {
            ...
            for sym in &all_symbols {
                ...
                if let Ok(embedding) = emb.embed(&embed_text) {
                    let _ = self.engine.storage.store_embedding(&sym_id, &embedding);
            ...
            for chunk in &all_chunks {
                ...
                let chunk_id = format!("chunk:{}:{}", chunk.file_path, chunk.index);
                if let Ok(embedding) = emb.embed(&embed_text) {
                    let _ = self.engine.storage.store_embedding(&chunk_id, &embedding);
        ...
        let (chunks_pruned, symbols_pruned) = if self.engine.config.chunking.auto_compact {
    ...
    fn compact_chunks(
        ...
        let max_chunks_per_file = self.engine.config.chunking.max_retained_chunks_per_file;
        let chunk_score_threshold = self.engine.config.chunking.min_chunk_score_threshold;
        ...
        for (file_path, mut chunks) in chunks_by_file {
            ...
            for (i, (chunk_id, score)) in chunks.iter().enumerate() {
                if i >= k || *score < chunk_score_threshold {
                    ...
                    let _ = self.engine.storage.delete_graph_edges_for_node(chunk_id);
                    let _ = self.engine.storage.delete_graph_node(chunk_id);
        ...
        if let Ok(Some(chunk_node)) = graph.get_node(chunk_id) {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                if let Ok(Some(mut parent_node)) = graph.get_node(&parent_id) {
                    ...
                    let _ = self.engine.storage.insert_graph_node(&parent_node);
    ...
    fn compact_symbols(
        ...
        let max_syms_per_file = self.engine.config.chunking.max_retained_symbols_per_file;
        let sym_score_threshold = self.engine.config.chunking.min_symbol_score_threshold;
        ...
        for (_file_path, mut syms) in syms_by_file {
            ...
            for (i, (sym_id, score, is_structural, mem_linked)) in syms.iter().enumerate() {
                ...
                let _ = self.engine.storage.delete_graph_edges_for_node(sym_id);
                let _ = self.engine.storage.delete_graph_node(sym_id);
            ...
            if let Some(fp) = sym_node.payload.get("file_path").and_then(|v| v.as_str()) {
                ...
                if let Ok(Some(mut file_node)) = graph.get_node(&file_id) {
                    ...
                    if line_end > line_start {
                        ...
                        let _ = self.engine.storage.insert_graph_node(&file_node);
...
/// Check if a graph node has any edge to a Memory node (not a code node).
