...
impl McpServer {
    ...
    pub(crate) fn tool_consolidate_decay(&self, args: &Value) -> ToolResult {
        ...
        let stale = match self
            .engine
            .storage
            .get_stale_memories_for_decay(threshold_ts)
        {
            Ok(rows) => rows,
        ...
        if stale.is_empty() {
            if let Err(e) = self.engine.storage.insert_consolidation_log("decay", 0) {
        ...
        let affected = match self.engine.storage.batch_update_importance(&updates) {
            Ok(count) => count,
        ...
        if let Err(e) = self
            .engine
            .storage
            .insert_consolidation_log("decay", affected)
        {
            tracing::warn!("Failed to log decay consolidation: {e}");
        }
    ...
    pub(crate) fn tool_consolidate_creative(&self, args: &Value) -> ToolResult {
        ...
        let parsed = match self.engine.storage.list_memories_for_creative() {
            Ok(rows) => rows,
        ...
        let memories = match self.engine.storage.get_memories_batch(&ids_refs) {
            Ok(m) => m,
            Err(e) => return ToolResult::tool_error(format!("Creative cycle failed: {e}")),
        };
        ...
        let all_edges = match self.engine.storage.all_graph_edges() {
            Ok(e) => e,
            Err(e) => return ToolResult::tool_error(format!("Creative cycle failed: {e}")),
        };
        ...
        // Load all embeddings
        let all_embeddings = match self.engine.storage.list_all_embeddings() {
            Ok(e) => e,
            Err(e) => return ToolResult::tool_error(format!("Creative cycle failed: {e}")),
        };
        ...
        for (id, embedding) in &embedding_map {
            ...
            for (neighbor_id, sim) in &neighbors {
                ...
                if self.engine.storage.insert_graph_edge(&edge).is_ok() {
    ...
    pub(crate) fn tool_consolidate_cluster(&self, args: &Value) -> ToolResult {
        ...
        let memories = match self.engine.storage.get_memories_batch(&id_refs) {
            Ok(m) => m,
            Err(e) => return ToolResult::tool_error(format!("Cluster cycle failed: {e}")),
        };
    ...
    pub(crate) fn tool_consolidate_forget(&self, args: &Value) -> ToolResult {
        ...
        let ids = if target_tags.is_empty() {
            match self.engine.storage.find_forgettable(importance_threshold) {
                Ok(ids) => ids,
        ...
        if let Err(e) = self
            .engine
            .storage
            .insert_consolidation_log("forget", deleted)
        {
    ...
    pub(crate) fn tool_consolidation_status(&self) -> ToolResult {
        let runs = match self.engine.storage.last_consolidation_runs() {
            Ok(r) => r,
    ...
    pub(crate) fn rebuild_vector_index_internal(&self, vector: &mut HnswIndex) {
        let embeddings = match self.engine.storage.list_all_embeddings() {
            Ok(e) => e,
            Err(e) => {
    ...
    pub(crate) fn tool_consolidate_summarize(&self, args: &Value) -> ToolResult {
        ...
        let provider = codemem_engine::compress::CompressProvider::from_env();
        ...
        for cluster in &large_clusters {
            ...
            for node_id in *cluster {
                ...
                if let Ok(Some(node)) = graph.get_node(node_id) {
                    if let Some(mid) = &node.memory_id {
                        let mid = mid.clone();
                        drop(graph);
                        if let Ok(Some(mem)) = self.engine.storage.get_memory(&mid) {
            ...
            // Persist via shared pipeline (storage + BM25 + graph + embedding)
    ...
    fn find_forgettable_by_tags(
        ...
        let all_memories = self.engine.storage.list_memories_filtered(None, None)?;
