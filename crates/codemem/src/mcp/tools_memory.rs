...
use super::scoring::{compute_score, format_recall_results, truncate_str};
...
impl McpServer {
    /// Persist a MemoryNode across all subsystems: storage, BM25, graph, embedding, vector index.
    /// Returns Ok(()) on success, or Err(ToolResult) for duplicate/error early return.
    pub(crate) fn persist_memory(&self, memory: &MemoryNode) -> Result<(), ToolResult> {
        // 1. Insert into storage
        match self.engine.storage.insert_memory(memory) {
            Ok(()) => {}
            Err(CodememError::Duplicate(h)) => {
                return Err(ToolResult::text(format!(
                    "Memory already exists (hash: {h})"
                )));
            }
            Err(e) => return Err(ToolResult::tool_error(format!("Storage error: {e}"))),
        }

        // 2. Update BM25 index
        match self.lock_bm25() {
            Ok(mut bm25) => bm25.add_document(&memory.id, &memory.content),
            Err(e) => return Err(ToolResult::tool_error(format!("Lock error: {e}"))),
        }
        ...
        let graph_node = GraphNode {
            ...
            label: truncate_str(&memory.content, 80),
            payload: HashMap::new(),
            centrality: 0.0,
            memory_id: Some(memory.id.clone()),
            namespace: None,
        };
        if let Err(e) = self.engine.storage.insert_graph_node(&graph_node) {
        ...
        // 4. Generate contextual embedding and insert into vector index
        if let Some(emb_guard) = match self.lock_embeddings() {
            ...
            match emb_guard.embed(&enriched) {
                Ok(embedding) => {
                    drop(emb_guard);
                    if let Err(e) = self.engine.storage.store_embedding(&memory.id, &embedding) {
                        tracing::warn!("Failed to store embedding: {e}");
                    }
                    match self.lock_vector() {
                        Ok(mut vec) => {
                            if let Err(e) = vec.insert(&memory.id, &embedding) {
    ...
    /// Core recall logic: search storage with hybrid scoring and return structured results.
    /// Used by both the MCP `recall_memory` tool and the REST API `search_memories` endpoint.
    ...
    pub fn recall(
        ...
        let candidate_ids: Vec<(String, f64)> = if vector_results.is_empty() {
            ...
            let ids = self.engine.storage.list_memory_ids()?;
            ids.into_iter().map(|id| (id, 0.0)).collect()
        } else {
            vector_results
                .iter()
                .map(|(id, distance)| (id.clone(), 1.0 - (*distance as f64)))
                .collect()
        };
    ...
    /// Search with optional type, namespace, and quality filters, returning a formatted ToolResult.
    #[allow(clippy::too_many_arguments)]
    ...
    pub(crate) fn tool_update_memory(&self, args: &Value) -> ToolResult {
        ...
        if let Err(e) = self.engine.storage.update_memory(id, content, importance) {
        ...
        if let Some(emb_guard) = match self.lock_embeddings() {
            ...
            let (mem_type, tags, namespace) =
                if let Ok(Some(mem)) = self.engine.storage.get_memory(id) {
    ...
    pub(crate) fn tool_delete_memory(&self, args: &Value) -> ToolResult {
        ...
        match self.engine.storage.delete_memory(id) {
            Ok(true) => {
    ...
    pub(crate) fn tool_refine_memory(&self, args: &Value) -> ToolResult {
        ...
        // Fetch old memory
        let old_memory = match self.engine.storage.get_memory(old_id) {
            Ok(Some(m)) => m,
    ...
    pub(crate) fn tool_split_memory(&self, args: &Value) -> ToolResult {
        ...
        let source_memory = match self.engine.storage.get_memory(source_id) {
            Ok(Some(m)) => m,
    ...
    pub(crate) fn tool_merge_memories(&self, args: &Value) -> ToolResult {
        ...
        let found = match self.engine.storage.get_memories_batch(&id_refs) {
            Ok(m) => m,
            Err(e) => return ToolResult::tool_error(format!("Storage error: {e}")),
        };
