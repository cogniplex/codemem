...
fn doctor_vector_index_creation() {
    let vector = codemem_engine::HnswIndex::with_defaults().unwrap();
...
fn doctor_vector_index_save_and_load() {
    ...
    let mut vector = codemem_engine::HnswIndex::with_defaults().unwrap();
    ...
    let mut loaded = codemem_engine::HnswIndex::with_defaults().unwrap();
