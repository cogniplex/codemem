...
pub struct AppState {
    ...
    pub watch_events: broadcast::Sender<codemem_watch::WatchEvent>,
...
impl ApiServer {
    ...
    pub fn new(server: Arc<McpServer>, storage: codemem_storage::Storage) -> Self {
        ...
        let (watch_tx, _) = broadcast::channel::<codemem_watch::WatchEvent>(256);
    ...
    pub fn router_with_mcp(&self) -> Router {
        let mcp = codemem_mcp::http::mcp_router(Arc::clone(&self.state.server));
