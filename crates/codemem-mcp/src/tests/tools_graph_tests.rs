...
fn graph_strength_increases_with_code_edges() {
    ...
    server.storage.insert_graph_node(&sym_node).unwrap();
    server.storage.insert_graph_node(&file_node).unwrap();
    {
...
fn graph_traverse_with_exclude_kinds() {
    ...
    server.storage.insert_graph_node(&file_node).unwrap();
    server.storage.insert_graph_node(&func_node).unwrap();
    server.storage.insert_graph_node(&chunk_node).unwrap();
    {
        let mut graph = server.graph.lock().unwrap();
        graph.add_node(file_node).unwrap();
...
fn compaction_prunes_low_score_chunks() {
    ...
    for i in 0..15 {
        ...
        let _ = server.storage.insert_graph_node(&node);
        let _ = graph.add_node(node);
        ...
        let _ = server.storage.insert_graph_edge(&edge);
        let _ = graph.add_edge(edge);
    }
    drop(graph);
...
fn compaction_preserves_memory_linked_chunks() {
    ...
    let _ = server.storage.insert_graph_edge(&link_edge);
...
fn compaction_prunes_low_score_symbols() {
    ...
    let _ = server.storage.insert_graph_node(&file_node);
