...
- **MCP tool consolidation**: 28 primary tools (down from 43), with legacy aliases for backwards compatibility. Key merges: `recall` (unified recall/expansion/impact), `consolidate` (unified 6 modes), `codemem_status` (unified stats/health/metrics), `search_code` (unified semantic/text/hybrid), `get_symbol_graph` (unified deps/impact)
